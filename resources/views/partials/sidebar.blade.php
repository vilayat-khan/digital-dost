            ...
            @foreach(($trending ?? collect())->take(5) as $item)
                <a href="{{ route('post.show', $item->slug) }}" style="display:grid; grid-template-columns:28px 1fr; gap:12px; align-items:start;">
                    ...
                    <span>
                        ...
                        <span class="muted" style="display:block; font-size:.82rem; margin-top:3px;">
                            {{ optional($item->author)->display_name }} · {{ optional($item->published_at)?->format('d M Y') }}
                        </span>
                    </span>
                </a>
            @endforeach
            ...
            @foreach(($latestReviews ?? collect())->take(4) as $item)
                <a href="{{ route('post.show', $item->slug) }}" style="display:block;">
                    ...
                    <div class="muted" style="font-size:.82rem; margin-top:3px;">
                        {{ optional($item->author)->display_name }}
                    </div>
                </a>
            @endforeach
        </div>
