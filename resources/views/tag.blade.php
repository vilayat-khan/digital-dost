...
@php
    ...
    $seoTitle = '#' . $tag->name . ' — Digital Dost';
    $seoDescription = 'Posts tagged with ' . $tag->name . ' on Digital Dost.';
...
@section('og_image_alt', '#' . $tag->name . ' — Digital Dost')
...
@section('twitter_image_alt', '#' . $tag->name . ' — Digital Dost')
...
<div class="container">
    <nav style="display:flex; gap:8px; flex-wrap:wrap; font-size:.8rem; color:var(--color-text-faint); margin-bottom:18px;">
        <a href="{{ url('/') }}">Home</a>
        <span>/</span>
        <span style="color:var(--color-text-muted);">#{{ $tag->name }}</span>
    </nav>
